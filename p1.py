...
def convolve(img, filt):
    ...
    init = np.zeros((m, n)) if isinstance(
        img[0][0], list) else np.zeros((m, n, 3))
...
def gradient(img):
    ...
    for i in range(n):
        for j in range(m):
            image_gradient[i][j] = np.sqrt(
                x_gradient[i][j] ** 2 + y_gradient[i][j] ** 2)
            image_orientation[i][j] = np.arctan2(
                y_gradient[i][j], x_gradient[i][j])
...
def draw_lines(img, lines, thresh):
    ...
    for i in range(n):
        for j in range(m):
            result_img[i][j] = [255, 0, 0] if any([check_distance_from_line(
                i, j, line, thresh) for line in lines]) else result_img[i][j]
    return result_img
...
def hough_voting(gradmag, gradori, thetas, cs, thresh1, thresh2, thresh3):
    ...
    for i in range(len(ans)):
        for j in range(len(ans[0])):
            ...
            part_c = [theta - gradori[y][x] <
                      thresh3 for x, y in zip(xpos, ypos)]
